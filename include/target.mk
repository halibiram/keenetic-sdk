...
ifeq ($(DUMP),)
  ...
  PLATFORM_DIR:=${CURDIR}
  ifeq ($(SUBTARGETS),)
    SUBTARGETS:=$(sort $(strip $(patsubst $(PLATFORM_DIR)/%/target.mk,%,$(wildcard $(PLATFORM_DIR)/*/target.mk))))
  endif
endif
...
ifneq ($(TARGET_BUILD),1)
  ifndef DUMP
    ...
    ifneq ($(PROFILE),)
      -include $(PLATFORM_SUBDIR)/profiles/$(PROFILE).mk
    endif
  endif
else
  ifneq ($(SUBTARGET),)
  ...
  ifneq ($(PROFILE),)
    -include ./$(SUBTARGET)/profiles/$(PROFILE).mk
  endif
endif
...
define Profile
  ...
  DUMPINFO += \
	...
	echo "Target-Profile-Linux-Version: $(VERSION)"; \
...
GENERIC_PATCH_DIR := $(GENERIC_PLATFORM_DIR)/patches$(if $(wildcard $(GENERIC_PLATFORM_DIR)/patches-$(LINUX_VERSION)),-$(LINUX_VERSION))
...
ifeq ($(DUMP),1)
  ...
  ifneq ($(BOARD),)
    ...
    ifneq ($(CONFIG_MT7615_AP),)
      FEATURES += radio_mt7615
    endif
    ifneq ($(CONFIG_MT7628_AP),)
      FEATURES += radio_mt7628
    endif
    ifneq ($(CONFIG_MT76X2_AP),)
      FEATURES += radio_mt76x2
    endif
  ...
  DEFAULT_CFLAGS=$(if $(DEFAULT_CFLAGS_$(ARCH)),$(DEFAULT_CFLAGS_$(ARCH)),-Os -pipe -fno-caller-saves) $(BOARD_CFLAGS)
endif
...
define BuildTargets/DumpCurrent
  ...
  dumpinfo:
	@echo 'Target: $(TARGETID)'; \
	 echo 'Target-Board: $(if $(SUBTARGET),$(SUBTARGET),$(BOARD))'; \
	 echo 'Target-Name: $(DESCRIPTION)$(if $(SUBTARGETS),$(if $(SUBTARGET),))'; \
	 echo 'Target-Path: $(subst $(TOPDIR)/,,$(PWD))'; \
	 echo 'Target-Arch: $(ARCH)'; \
	 echo 'Target-Arch-Packages: $(if $(ARCH_PACKAGES),$(ARCH_PACKAGES),$(if $(SUBTARGET),$(SUBTARGET)))'; \
	 echo 'Target-Features: $(FEATURES)'; \
	 echo 'Target-Depends: $(DEPENDS)'; \
	 echo 'Target-Optimization: $(if $(CFLAGS),$(CFLAGS),$(DEFAULT_CFLAGS))'; \
	 echo 'Target-CPPFLAGS: $(if $(BOARD_CPPFLAGS),$(BOARD_CPPFLAGS))'; \
	 echo 'Linux-Version: $(LINUX_VERSION)'; \
	 echo 'Linux-Release: $(LINUX_RELEASE)'; \
	 echo 'Linux-Kernel-Arch: $(LINUX_KARCH)'; \
	$(if $(filter private,$(FLAGS)),echo 'Private: 1';) \
...
ifeq ($(TARGET_BUILD),1)
  ...
  TMP_COMPONENTS_IN_PRESETS:=$(TMP_DIR)/.components-in-presets
