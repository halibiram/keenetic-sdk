...
define KernelPackage/nf-nat6
  SUBMENU:=$(NF_MENU)
  TITLE:=Basic netfilter IPV6-NAT modules
...
define KernelPackage/nf-nat6-extra
  SUBMENU:=$(NF_MENU)
  TITLE:=Extra netfilter IPV6-NAT modules
  KCONFIG:=$(KCONFIG_NF_NAT6_EXTRA)
  DEPENDS:=+kmod-nf-nat6
  FILES:=$(foreach mod,$(NF_NAT6_EXTRA-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoProbe,$(notdir $(NF_NAT6_EXTRA-m)))
endef

$(eval $(call KernelPackage,nf-nat6-extra))
...
define KernelPackage/ipt-ipopt-extended
  TITLE:=Modules for matching/changing IP packet options
  KCONFIG:=$(KCONFIG_IPT_IPOPT_EXTENDED)
  FILES:=$(foreach mod,$(IPT_IPOPT_EXTENDED-m),$(LINUX_DIR)/net/$(mod).ko)
  AUTOLOAD:=$(call AutoLoad,45,$(notdir $(IPT_IPOPT_EXTENDED-m)))
  $(call AddDepends/ipt)
endef

define KernelPackage/ipt-ipopt-extended/description
 Additional netfilter (IPv4) modules for matching/changing IP packet options
 Includes:
 - CLASSIFY
 - ecn/ECN
 - hl
...
$(eval $(call KernelPackage,ipt-ipopt-extended))
...
define KernelPackage/ipt-ipset-base
  SUBMENU:=$(NF_MENU)
  TITLE:=IPset base netfilter modules 
...
$(eval $(call KernelPackage,ipt-ipset-base))
...
define KernelPackage/nf-conntrack-netlink
  ...
  KCONFIG:=CONFIG_NF_CT_NETLINK=y
...
define KernelPackage/ipt-conntrack-raw
  SUBMENU:=$(NF_MENU)
  TITLE:=Netfilter conntrack extra modules
  KCONFIG:=CONFIG_NETFILTER_XT_TARGET_NOTRACK=m \
	CONFIG_NETFILTER_XT_TARGET_CT=m
  DEPENDS:=+kmod-ipt-raw
  FILES:=$(LINUX_DIR)/net/netfilter/xt_CT.ko
endef

define KernelPackage/ipt-conntrack-raw/description
 Kernel module for RAW conntrack handling
endef

$(eval $(call KernelPackage,ipt-conntrack-raw))
