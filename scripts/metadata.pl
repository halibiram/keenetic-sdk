...
sub target_config_features(@) {
	...
	while ($_ = shift @_) {
		...
		/^usbstorage$/ and $ret .= "\tselect USB_STORAGE_SUPPORT\n";
...
sub target_name($) {
	...
	if ($parent) {
		print "Returning " .$target->{parent}->{name}." - ".$target->{name}."\n";
		return $target->{parent}->{name}." - ".$target->{name};
	} else {
		print "Returning +++ $target->{name}\n";
		return $target->{name};
	}
}
...
sub print_target($) {
	...
	if (@{$target->{subtargets}} == 0 && $target->{only_default_profile}) {
	$confstr = <<EOF;
...
EOF
			...
			my $v = $target->{version};
			if ($profile->{version} =~ /\w/) {
				$v = $profile->{version};
			}
			print "\tselect LINUX_".kver($v)."\n";

			my @pkglist = merge_package_lists($target->{packages}, $profile->{packages});
			foreach my $pkg (@pkglist) {
#				print "\tselect DEFAULT_$pkg\n";
				print "\tselect PACKAGE_$pkg\n";
				$defaults{$pkg} = 1;
			}
...
EOF
	foreach my $target (@target) {
		$target->{subtarget} or	print "\t\tdefault \"".$target->{conf}."\" if TARGET_".$target->{conf}."\n";
	}
	print <<EOF;
...
EOF
	foreach my $target (@target) {
		next unless $target->{subtarget};
		print "\t\tdefault \"".$target->{name}."\" if TARGET_".$target->{conf}."\n";
	}
...
EOF
	...
	print <<EOF;
config BOARD_CPPFLAGS
	string
EOF
	foreach my $target (@target) {
		next if @{$target->{subtargets}} > 0;
		print "\tdefault \"".$target->{cppflags}."\" if TARGET_".$target->{conf}."\n";
	}
	my %kver;
	foreach my $target (@target) {
		my $v = kver($target->{version});
...
sub gen_package_mk() {
	...
	foreach my $name (sort {uc($a) cmp uc($b)} keys %package) {
		...
		$multi_package = 1 if @{$srcpackage{$pkg->{src}}} > 1;
		...
		foreach my $deps (@srcdeps) {
			...
			foreach my $dep (@deps) {
				...
				if ($idx) {
					...
					if (!$vdep && $multi_package) {
						$depline = get_conditional_dep("PACKAGE_$srcnames[0]", $depline);
					}

					if ($config and $suffix eq '/host') {
						print "package-$config += \$(if \$(filter package/download,\$(MAKECMDGOALS)),$subdir{$dep}$dep)\n";
					}
...
sub gen_strip_description($) {
	my $description = shift;

	$description =~ s/^\s*//g;
	$description =~ s/^.*<.*@.*>.*$//mg; #delete MAINTAINER
	$description =~ s/[\r\n]\s*/ /mg;
...
sub gen_package_legalcsv() {
	...
	foreach my $name (sort {uc($a) cmp uc($b)} keys %package) {
		...
		if ($pkg->{name} &&
			$pkg->{license} &&
			$pkg->{license} ne "NDM" &&
			$pkg->{version} ne "<LINUX_VERSION>-")
		{
			print "$pkg->{name};";
			print "$pkg->{license};";
			print "$pkg->{version};";
			print gen_strip_description($pkg->{description});
			print "\n";
		}
	}
}
